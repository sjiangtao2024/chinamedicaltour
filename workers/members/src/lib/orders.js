...
export async function updateOrderPayment(db, orderId, updates) {
  ...
  await db
    .prepare(
      "UPDATE orders SET paypal_order_id = COALESCE(?, paypal_order_id), paypal_capture_id = COALESCE(?, paypal_capture_id), payment_gateway_fee = COALESCE(?, payment_gateway_fee), payment_channel = COALESCE(?, payment_channel), transaction_id = COALESCE(?, transaction_id), service_status = COALESCE(?, service_status), status = ?, updated_at = ? WHERE id = ?"
    )
    .bind(
