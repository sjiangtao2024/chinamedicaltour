...
const migrationPath = (name) => new URL(`../migrations/${name}`, import.meta.url);

const sql = fs.readFileSync(migrationPath("0001_create_members_tables.sql"), "utf8");
assert.ok(sql.includes("CREATE TABLE users"));
assert.ok(sql.includes("CREATE TABLE orders"));

const profileSql = fs.readFileSync(
  migrationPath("0002_add_user_profiles.sql"),
  "utf8"
);
assert.ok(profileSql.includes("CREATE TABLE user_profiles"));

const couponIssuerSql = fs.readFileSync(
  migrationPath("0005_add_coupon_issuer.sql"),
  "utf8"
);
...
const refundPolicySql = fs.readFileSync(
  migrationPath("0010_add_refund_policy_fields.sql"),
  "utf8"
);
...
const agreementSql = fs.readFileSync(
  migrationPath("0011_create_agreement_acceptances.sql"),
  "utf8"
);
